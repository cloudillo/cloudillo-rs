...
pub async fn get_access_token(
	...
	if let Some(token_param) = query.token {
		...
		let response = ApiResponse::new(json!({ "token": token_result })).with_req_id(req_id.unwrap_or_default());
		Ok((StatusCode::OK, Json(response)))
	} else {
...
pub async fn get_proxy_token(
	...
	let response = ApiResponse::new(ProxyTokenRes { token }).with_req_id(req_id.unwrap_or_default());
