...
pub fn check_perm_file(
	action: &'static str,
) -> impl Fn(State<App>, Auth, Path<String>, Request, Next) -> Pin<Box<dyn Future<Output = Result<Response, Error>> + Send>> + Clone {
	move |state, auth, path, req, next| Box::pin(check_file_permission(state, auth, path, req, next, action))
}
...
async fn load_file_attrs(_app: &App, _tn_id: TnId, _file_id: &str, _subject_id_tag: &str) -> ClResult<FileAttrs> {
