...
impl Condition {
    ...
    pub fn evaluate(
        ...
        match self.attribute.as_str() {
            "subject.id_tag" => {
                self.compare_value(&subject.id_tag)
            }
            "subject.tn_id" => {
                self.compare_value(&subject.tn_id.0.to_string())
            }
...
impl PolicyRule {
    ...
    pub fn evaluate(
        ...
        let all_match = self.conditions.iter()
