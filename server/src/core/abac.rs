...
impl Condition {
	...
	pub fn evaluate(&self, subject: &AuthCtx, action: &str, object: &dyn AttrSet, _environment: &Environment) -> bool {
...
impl PolicyRule {
	...
	pub fn evaluate(&self, subject: &AuthCtx, action: &str, object: &dyn AttrSet, environment: &Environment) -> Option<Effect> {
...
impl PermissionChecker {
	...
	pub fn has_permission(&self, subject: &AuthCtx, action: &str, object: &dyn AttrSet, environment: &Environment) -> bool {
		...
		if let Some(profile_policy) = self.profile_policies.get(&subject.tn_id) {
			if let Some(Effect::Deny) = profile_policy.top_policy.evaluate(subject, action, object, environment) {
				info!("TOP policy denied: tn_id={}, action={}", subject.tn_id.0, action);
				return false;
			}

			// Step 2: Check BOTTOM policy (guarantees)
			if let Some(Effect::Allow) = profile_policy.bottom_policy.evaluate(subject, action, object, environment) {
	...
	fn check_default_rules(&self, subject: &AuthCtx, action: &str, object: &dyn AttrSet, _environment: &Environment) -> bool {
		use tracing::debug;
