...
pub async fn init(state: App, acme_email: &str, id_tag: &str, app_domain: Option<&str>) -> ClResult<()> {
	...
	let (account, credentials) = Account::builder()?
		.create(
			&acme::NewAccount { contact: &[], terms_of_service_agreed: true, only_return_existing: false },
...
pub async fn get_acme_challenge(State(state): State<App>, headers: HeaderMap) -> ClResult<Box<str>> {
	let domain = headers
		.get("host")
