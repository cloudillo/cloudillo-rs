...
/// Extract IP from SocketAddr for non-HTTP contexts
pub fn extract_client_ip_from_socket(addr: &SocketAddr, mode: &ServerMode) -> IpAddr {
	match mode {
		// In standalone mode, socket address is the real client
		ServerMode::Standalone => addr.ip(),
		// In proxy mode, socket is the proxy - but we don't have headers here
		// so we fall back to socket address (caller should use request-based extraction when possible)
		ServerMode::Proxy | ServerMode::StreamProxy => addr.ip(),
	}
}
