...
mod tests {
	...
	fn test_ipv4_detection() {
		assert_eq!(parse_address_type("192.168.1.1").unwrap(), AddressType::Ipv4);
		assert_eq!(parse_address_type("203.0.113.42").unwrap(), AddressType::Ipv4);
		assert_eq!(parse_address_type("0.0.0.0").unwrap(), AddressType::Ipv4);
		assert_eq!(parse_address_type("255.255.255.255").unwrap(), AddressType::Ipv4);
	}

	#[test]
	fn test_ipv6_detection() {
		assert_eq!(parse_address_type("2001:db8::1").unwrap(), AddressType::Ipv6);
		assert_eq!(parse_address_type("::1").unwrap(), AddressType::Ipv6);
		assert_eq!(parse_address_type("::").unwrap(), AddressType::Ipv6);
		assert_eq!(parse_address_type("fe80::1").unwrap(), AddressType::Ipv6);
	}

	#[test]
	fn test_hostname_detection() {
		assert_eq!(parse_address_type("example.com").unwrap(), AddressType::Hostname);
		assert_eq!(parse_address_type("server.cloudillo.net").unwrap(), AddressType::Hostname);
		assert_eq!(parse_address_type("api-server").unwrap(), AddressType::Hostname);
		assert_eq!(parse_address_type("my_server").unwrap(), AddressType::Hostname);
	}
	...
	fn test_address_type_consistency_empty() {
		...
		assert_eq!(validate_address_type_consistency(&addresses).unwrap(), None);
	}

	#[test]
	fn test_address_type_consistency_single() {
		let addresses = vec!["192.168.1.1".into()];
		let result = validate_address_type_consistency(&addresses).unwrap();
		assert_eq!(result, Some(AddressType::Ipv4));
	}

	#[test]
	fn test_address_type_consistency_all_ipv4() {
		let addresses = vec!["192.168.1.1".into(), "10.0.0.1".into(), "172.16.0.1".into()];
		let result = validate_address_type_consistency(&addresses).unwrap();
		assert_eq!(result, Some(AddressType::Ipv4));
	}

	#[test]
	fn test_address_type_consistency_all_ipv6() {
		let addresses = vec!["2001:db8::1".into(), "::1".into(), "fe80::1".into()];
		let result = validate_address_type_consistency(&addresses).unwrap();
		assert_eq!(result, Some(AddressType::Ipv6));
	}

	#[test]
	fn test_address_type_consistency_all_hostname() {
		let addresses = vec!["example.com".into(), "api.example.com".into(), "server".into()];
		let result = validate_address_type_consistency(&addresses).unwrap();
		assert_eq!(result, Some(AddressType::Hostname));
	}

	#[test]
	fn test_address_type_consistency_mixed_ipv4_ipv6() {
		let addresses = vec!["192.168.1.1".into(), "2001:db8::1".into()];
		assert!(validate_address_type_consistency(&addresses).is_err());
	}

	#[test]
	fn test_address_type_consistency_mixed_ipv4_hostname() {
		let addresses = vec!["192.168.1.1".into(), "example.com".into()];
		assert!(validate_address_type_consistency(&addresses).is_err());
	}

	#[test]
	fn test_address_type_consistency_mixed_ipv6_hostname() {
		let addresses = vec!["2001:db8::1".into(), "example.com".into()];
		assert!(validate_address_type_consistency(&addresses).is_err());
	}

	#[test]
	fn test_address_type_consistency_mixed_all_three() {
		let addresses = vec!["192.168.1.1".into(), "2001:db8::1".into(), "example.com".into()];
		assert!(validate_address_type_consistency(&addresses).is_err());
	}
}
