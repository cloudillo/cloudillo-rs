...
// Re-export from cloudillo-core for backward compatibility
pub use cloudillo_core::bootstrap_types::CreateCompleteTenantOptions;
...
pub async fn bootstrap(app: Arc<AppState>, opts: &crate::app::AppBuilderOpts) -> ClResult<()> {
	...
	// Schedule profile refresh batch task
	{
		let app_clone = app.clone();
		tokio::spawn(async move {
			let refresh_task = Arc::new(cloudillo_profile::sync::ProfileRefreshBatchTask);
			match app_clone
				.scheduler
				.task(refresh_task)
