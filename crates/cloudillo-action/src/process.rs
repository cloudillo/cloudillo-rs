...
pub async fn verify_action_token(
	...
	if let Some(failure) = key_cache.check_failure(issuer, key_id) {
		debug!(
	...
	match fetch_result {
		Ok(api_response) => {
			...
			if let Some(key) = key {
				...
				key_cache.clear_failure(issuer, key_id);
				...
				key_cache.record_failure(issuer, key_id, &err);
		...
		Err(e) => {
			...
			key_cache.record_failure(issuer, key_id, &e);
			Err(e)
		}
	}
}
