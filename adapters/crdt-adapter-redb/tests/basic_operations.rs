...
async fn test_metadata_operations() {
	...
	assert_eq!(
		retrieved.custom["title"],
		serde_json::json!("My Document")
	);
}
...
async fn test_per_tenant_isolation() {
	...
	let update1 = CrdtUpdate {
		data: vec![0x11],
		timestamp: 1698499200,
		client_id: None,
	};

	let update2 = CrdtUpdate {
		data: vec![0x22],
		timestamp: 1698499300,
		client_id: None,
	};

	adapter
		.store_update(TnId(1), doc_id, update1)
		.await
		.expect("Failed to store");

	adapter
		.store_update(TnId(2), doc_id, update2)
		.await
		.expect("Failed to store");

	let updates1 = adapter
		.get_updates(TnId(1), doc_id)
		.await
		.expect("Failed to get");

	let updates2 = adapter
		.get_updates(TnId(2), doc_id)
		.await
		.expect("Failed to get");
