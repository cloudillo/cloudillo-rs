...
async fn create_test_adapter() -> (CrdtAdapterRedb, TempDir) {
	...
	let config = AdapterConfig { max_instances: 10, idle_timeout_secs: 60, broadcast_capacity: 100, auto_evict: false };
...
async fn test_create_and_store_update() {
	...
	let update = CrdtUpdate {
		...
		timestamp: cloudillo::prelude::Timestamp(1698499200),
	...
	assert!(updates[0].timestamp > cloudillo::prelude::Timestamp(0));
...
async fn test_multiple_updates() {
	...
	for i in 1..=3 {
		let update = CrdtUpdate {
			data: vec![i as u8],
			timestamp: cloudillo::prelude::Timestamp((1698499200 + i as u64).try_into().unwrap()),
			client_id: None,
		};
...
async fn test_delete_document() {
	...
	// Store an update
	let update = CrdtUpdate { data: vec![0xFF], timestamp: cloudillo::prelude::Timestamp(1698499200), client_id: None };
...
async fn test_per_tenant_isolation() {
	...
	let update1 = CrdtUpdate { data: vec![0x11], timestamp: cloudillo::prelude::Timestamp(1698499200), client_id: None };

	let update2 = CrdtUpdate { data: vec![0x22], timestamp: cloudillo::prelude::Timestamp(1698499300), client_id: None };
...
async fn test_large_binary_update() {
	...
	let update = CrdtUpdate { data: large_data.clone(), timestamp: cloudillo::prelude::Timestamp(1698499200), client_id: None };
