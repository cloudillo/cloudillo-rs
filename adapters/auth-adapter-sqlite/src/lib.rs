...
use cloudillo::{action::action, auth_adapter, core::utils::random_id, core::worker::WorkerPool, meta_adapter, prelude::*};
...
impl auth_adapter::AuthAdapter for AuthAdapterSqlite {
	async fn validate_token(&self, tn_id: TnId, id_tag: &str, token: &str) -> ClResult<auth_adapter::AuthCtx> {
		let token_data = decode::<auth_adapter::AccessToken<Box<str>>>(token, &self.jwt_secret, &Validation::new(Algorithm::HS256))
	...
	async fn create_tenant_registration(&self, email: &str) -> ClResult<()> {
		...
		sqlx::query("INSERT OR REPLACE INTO user_vfy (vfy_code, email, func) VALUES (?1, ?2, 'register')")
			.bind(&vfy_code)
			.bind(email)
			.execute(&self.db)
			.await
	...
	async fn create_tenant(&self, id_tag: &str, email: Option<&str>, vfy_code: Option<&str>) -> ClResult<TnId> {
		...
		let res = sqlx::query("INSERT INTO tenants (id_tag, email, status) VALUES (?1, ?2, 'A') RETURNING tn_id")
			.bind(id_tag)
			.bind(email)
			.fetch_one(&self.db)
			.await;
	...
	async fn read_cert_by_tn_id(&self, tn_id: TnId) -> ClResult<auth_adapter::CertData> {
		let res = sqlx::query("SELECT tn_id, id_tag, domain, cert, key, expires_at FROM certs WHERE tn_id = ?1")
			.bind(tn_id.0)
			.fetch_one(&self.db)
			.await;
	...
	async fn read_cert_by_id_tag(&self, id_tag: &str) -> ClResult<auth_adapter::CertData> {
		let res = sqlx::query("SELECT tn_id, id_tag, domain, cert, key, expires_at FROM certs WHERE id_tag = ?1")
			.bind(id_tag)
			.fetch_one(&self.db)
			.await;
	...
	async fn read_cert_by_domain(&self, domain: &str) -> ClResult<auth_adapter::CertData> {
		let res = sqlx::query("SELECT tn_id, id_tag, domain, cert, key, expires_at FROM certs WHERE domain = ?1")
			.bind(domain)
			.fetch_one(&self.db)
			.await;
	...
	async fn read_profile_key(&self, tn_id: TnId, key_id: &str) -> ClResult<auth_adapter::AuthKey> {
		let res = sqlx::query("SELECT key_id, public_key, expires_at FROM keys WHERE tn_id = ?1 AND key_id = ?2")
			.bind(tn_id.0)
			.bind(key_id)
			.fetch_optional(&self.db)
			.await
	...
	async fn create_profile_key(&self, tn_id: TnId, expires_at: Option<Timestamp>) -> ClResult<auth_adapter::AuthKey> {
		...
		sqlx::query("INSERT INTO keys (tn_id, key_id, private_key, public_key, expires_at) VALUES (?1, ?2, ?3, ?4, ?5)")
			.bind(tn_id.0)
			.bind(&key_id)
		...
		Ok(auth_adapter::AuthKey { key_id: key_id.into(), public_key: keypair.public_key, expires_at })
	}
	...
	async fn verify_access_token(&self, token: &str) -> ClResult<()> {
		...
		decode::<auth_adapter::AccessToken<Box<str>>>(token, &self.jwt_secret, &Validation::new(Algorithm::HS256))
			.map_err(|_| Error::Unauthorized)?;
	...
	async fn read_vapid_key(&self, tn_id: TnId) -> ClResult<auth_adapter::KeyPair> {
		let res = sqlx::query("SELECT vapid_public_key, vapid_private_key FROM tenants WHERE tn_id = ?1")
			.bind(tn_id.0)
			.fetch_optional(&self.db)
			.await
			.inspect_err(inspect)
		...
		match (public_key, private_key) {
			(Some(pub_key), Some(priv_key)) => Ok(auth_adapter::KeyPair { public_key: pub_key.into(), private_key: priv_key.into() }),
	...
	async fn update_vapid_key(&self, tn_id: TnId, key: &auth_adapter::KeyPair) -> ClResult<()> {
		sqlx::query("UPDATE tenants SET vapid_public_key = ?1, vapid_private_key = ?2 WHERE tn_id = ?3")
	...
	async fn update_var(&self, tn_id: TnId, var: &str, value: &str) -> ClResult<()> {
		let key = format!("{}:{}", tn_id.0, var);
		sqlx::query("INSERT OR REPLACE INTO vars (key, value, updated_at) VALUES (?1, ?2, current_timestamp)")
			.bind(&key)
			.bind(value)
			.execute(&self.db)
			.await
	...
	async fn list_webauthn_credentials(&self, tn_id: TnId) -> ClResult<Box<[auth_adapter::Webauthn]>> {
		let res = sqlx::query("SELECT credential_id, counter, public_key, description FROM webauthn WHERE tn_id = ?1")
			.bind(tn_id.0)
			.fetch_all(&self.db)
			.await
			.inspect_err(inspect)
	...
	async fn read_webauthn_credential(&self, tn_id: TnId, credential_id: &str) -> ClResult<auth_adapter::Webauthn> {
		let res =
			sqlx::query("SELECT credential_id, counter, public_key, description FROM webauthn WHERE tn_id = ?1 AND credential_id = ?2")
				.bind(tn_id.0)
				.bind(credential_id)
				.fetch_optional(&self.db)
				.await
	...
	async fn create_webauthn_credential(&self, tn_id: TnId, data: &auth_adapter::Webauthn) -> ClResult<()> {
		sqlx::query("INSERT INTO webauthn (tn_id, credential_id, counter, public_key, description) VALUES (?1, ?2, ?3, ?4, ?5)")
			.bind(tn_id.0)
	...
	async fn create_registration_verification(&self, email: &str) -> ClResult<Box<str>> {
		...
		sqlx::query("INSERT OR REPLACE INTO user_vfy (vfy_code, email, func, expires_at) VALUES (?1, ?2, 'register', ?3)")
			.bind(&vfy_code)
			.bind(email)
	...
	async fn validate_registration_verification(&self, email: &str, vfy_code: &str) -> ClResult<()> {
		let row = sqlx::query("SELECT email FROM user_vfy WHERE vfy_code = ?1 AND email = ?2 AND func = 'register'")
			.bind(vfy_code)
			.bind(email)
			.fetch_optional(&self.db)
			.await
...
async fn init_db(db: &SqlitePool) -> Result<(), sqlx::Error> {
	...
	let _ = sqlx::query("UPDATE tenants SET roles = NULL WHERE roles IS NULL OR roles = 'null' OR roles = '[]'")
		.execute(&mut *tx)
		.await;
