...
mod tests {
	...
	async fn test_read_vapid_public_key() {
		...
		let keypair = cloudillo::auth_adapter::KeyPair { public_key: test_public_key.into(), private_key: test_private_key.into() };
	...
	async fn test_read_vapid_key_pair() {
		...
		// Read full key pair
		let read_keypair = adapter.read_vapid_key(tn_id).await.expect("Failed to read VAPID key pair");
	...
	async fn test_update_vapid_key_overwrites() {
		...
		let keypair1 = cloudillo::auth_adapter::KeyPair { public_key: "key1-public".into(), private_key: "key1-private".into() };
		...
		// Update with new key
		let keypair2 = cloudillo::auth_adapter::KeyPair { public_key: "key2-public".into(), private_key: "key2-private".into() };
	...
	async fn test_vapid_key_per_tenant_isolation() {
		...
		let keypair1 = cloudillo::auth_adapter::KeyPair { public_key: "tenant1-public".into(), private_key: "tenant1-private".into() };

		let keypair2 = cloudillo::auth_adapter::KeyPair { public_key: "tenant2-public".into(), private_key: "tenant2-private".into() };
